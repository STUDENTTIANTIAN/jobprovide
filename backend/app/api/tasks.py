...
@router.get("/{task_id}")
async def get_task_detail(task_id: UUID, db: AsyncSession = Depends(get_db)):
    ...
    segments_result = await db.execute(
        select(SubtitleSegment).where(SubtitleSegment.task_id == task_id).order_by(SubtitleSegment.sort_order)
    )
    ...
    for segment in segments:
        matches_result = await db.execute(
            select(MatchResult).where(MatchResult.segment_id == segment.id).order_by(MatchResult.rank)
        )
        ...
        for match in matches:
            ...
            media_result = await db.execute(select(Media).where(Media.id == match.media_id))
...
async def get_task_segments(task_id: UUID, db: AsyncSession = Depends(get_db)):
...
async def update_segment(
    task_id: UUID, segment_id: UUID, content: Optional[str] = None,
    sort_order: Optional[int] = None, selected_media_id: Optional[UUID] = None,
    db: AsyncSession = Depends(get_db)
):
    service = SubtitleService(db)
    segment = await service.update_segment(segment_id, content=content, sort_order=sort_order, selected_media_id=selected_media_id)
    if not segment:
